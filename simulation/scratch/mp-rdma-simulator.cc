...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		MpRdmaClientHelper clientHelper(flow_input.pg, serverAddress[flow_input.src], serverAddress[flow_input.dst], port, flow_input.dport, flow_input.maxPacketCount, has_win?(global_t==1?maxBdp:pairBdp[n.Get(flow_input.src)][n.Get(flow_input.dst)]):0, global_t==1?maxRtt:pairRtt[flow_input.src][flow_input.dst]);
...
void monitor_rate() {
    Ptr<Node> h = Hosts[3];
    Ptr<MpRdmaQueuePairGroup> qpg = DynamicCast<MpQbbNetDevice>(h->GetDevice(1))->m_rdmaEQ->m_qpGrp;
    for(int i = 0; i < qpg->GetN(); i++) {
        Ptr<MpRdmaQueuePair> q = qpg->Get(i);
