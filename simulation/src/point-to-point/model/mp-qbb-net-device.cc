...
namespace ns3 {
	...
	uint32_t MpRdmaEgressQueue::GetNBytes(uint32_t qIndex){
		NS_ASSERT_MSG(qIndex < m_qpGrp->GetN(), "MpRdmaEgressQueue::GetNBytes: qIndex >= m_qpGrp->GetN()");
	...
	void MpRdmaEgressQueue::RecoverQueue(uint32_t i){
		NS_ASSERT_MSG(i < m_qpGrp->GetN(), "MpRdmaEgressQueue::RecoverQueue: qIndex >= m_qpGrp->GetN()");
		...
		MpQbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::MpQbbNetDevice")
			...
			.AddAttribute ("MpRdmaEgressQueue", 
					"A queue to use as the transmit queue in the device.",
					PointerValue (),
					MakePointerAccessor (&MpQbbNetDevice::m_rdmaEQ),
					MakePointerChecker<Object> ())
			.AddTraceSource ("QbbEnqueue", "Enqueue a packet in the MpQbbNetDevice.",
					MakeTraceSourceAccessor (&MpQbbNetDevice::m_traceEnqueue))
			.AddTraceSource ("QbbDequeue", "Dequeue a packet in the MpQbbNetDevice.",
					MakeTraceSourceAccessor (&MpQbbNetDevice::m_traceDequeue))
			.AddTraceSource ("QbbDrop", "Drop a packet in the MpQbbNetDevice.",
	...
	void
		MpQbbNetDevice::TransmitComplete(void)
	{
		NS_LOG_FUNCTION(this);
	...
	void
		MpQbbNetDevice::DequeueAndTransmit(void)
	{
		NS_LOG_FUNCTION(this);
	...
	{
		NS_ASSERT_MSG(false, "MpQbbNetDevice::Send not implemented yet\n");
		return false;
	}
    ...
    bool MpQbbNetDevice::MemsSend (Ptr<Packet> packet){
		m_macTxTrace(packet);
