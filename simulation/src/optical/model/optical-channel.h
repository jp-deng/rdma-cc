...
public:
   ...
   */
  void Attach (Ptr<OpticalNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source OpticalNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<Packet> p, Ptr<OpticalNetDevice> src, Time txTime);
  ...
  /*
   * \brief Get OpticalNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to OpticalNetDevice requested
   */
  Ptr<OpticalNetDevice> GetOpticalDevice (uint32_t i) const;
...
protected:
   ...
   * \returns Ptr to OpticalNetDevice source for the 
   * specified link
   */
  Ptr<OpticalNetDevice> GetSource (uint32_t i) const;

  /*
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to OpticalNetDevice destination for 
   * the specified link
   */
  Ptr<OpticalNetDevice> GetDestination (uint32_t i) const;
...
public:
    ...
    WireState                  m_state;
    Ptr<OpticalNetDevice> m_src;
    Ptr<OpticalNetDevice> m_dst;
  };
